...
from keras.layers import UpSampling2D, Conv2D, BatchNormalization, Activation, concatenate, Add, GlobalAveragePooling2D, Dense
from keras.utils import get_file

# from xception_padding import Xception
from models.resnets import ResNet101, ResNet152, ResNet50, ResNet50_VV2
...
def resnet50_fpn_custom(input_shape, channels=1, activation="softmax"):
    img_input = Input(input_shape)
    resnet_base = ResNet50(img_input, include_top=True)
    resnet_base.load_weights(download_resnet_imagenet("resnet50"))
    conv1 = resnet_base.get_layer("conv1_relu").output
    ...
    x_ = conv_bn_relu(x, 256, 3, (1, 1), name="aggregation_")
    x_out = GlobalAveragePooling2D()(x_)
    ...
    if activation == 'softmax':
        ...
        x = Conv2D(channels, (1, 1), activation=activation, name="mask")(x)
    model = Model(img_input, [x,x_out])
    return model

def resnet50_fpn_v2(input_shape, channels=1, activation="softmax"):
    img_input = Input(input_shape)
    resnet_base = ResNet50(img_input, include_top=True)
    #resnet_base.load_weights(download_resnet_imagenet("resnet50"))
    conv1 = resnet_base.get_layer("conv1_relu").output
...
def densenet121_fpn(input_shape, channels=1, activation="sigmoid"):
    densenet = keras.applications.densenet.DenseNet121(weights='imagenet', include_top=False, input_shape=input_shape)
...
def densenet121_fpn_thyroid_cls(input_shape, channels=1, activation="sigmoid"):
    model_path = '/data/AlgProj/guxl/Thyroid_experiment/model_thyroid_cam_cls/study55/cls_densenet121_lr-0.0001_bs-8_ep-009_loss-0.2998_val_loss-0.4525.hdf5'
...
def densenet121_fpn_thyroid_cls_hwr(input_shape, channels=1, activation="sigmoid"):
    model_path = '/data/AlgProj/guxl/Thyroid_experiment/model_thyroid_cam_cls/study67/exp-0_lr-0.0001_bs-8.hdf5'
    densenet = load_model(model_path)
    conv1 = densenet.get_layer("conv1/relu").output
    conv2 = densenet.get_layer("pool2_relu").output
    conv3 = densenet.get_layer("pool3_relu").output
...
def densenet121_fpn_thyroid_cls_video(input_shape, channels=1, activation="sigmoid"):
    model_path = '/data/AlgProj/guxl/Thyroid_experiment/model_thyroid_cls_video/study19/cls_densenet121_lr-0.0001_bs-8_ep-006_loss-0.3466_val_loss-0.4980.hdf5'
    densenet = load_model(model_path)
    conv1 = densenet.get_layer("conv1/relu").output
    conv2 = densenet.get_layer("pool2_relu").output
    conv3 = densenet.get_layer("pool3_relu").output
