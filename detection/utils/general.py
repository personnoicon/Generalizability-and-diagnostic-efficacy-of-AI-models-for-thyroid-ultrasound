...
def ap_per_classv2(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=()):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
    # Arguments
        tp:  True positives (nparray, nx1 or nx10).
    ...
    if plot:
        plot_pr_curve(px, py, Path(save_dir) / 'PR_curve.png', names)
...
from albumentations import CenterCrop, Resize, Normalize, Compose
...
def get_valid_transforms_cv2(img_size):
    return Compose([
        Cv2Resize(img_size, img_size, interpolation=cv2.INTER_LINEAR),
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                # im = cv2.resize(cutout, (224, 224), cv2.INTER_LINEAR)  # BGR
                # cv2.imwrite('/home/projects/yolov5-3.1/inference/exp15_v5s-416/test%i.jpg' % j, cutout)
                # cutout = cutout.transpose(2, 0, 1)
                # im = im[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
                # im = np.ascontiguousarray(im, dtype=np.float32)  # uint8 to float32
                # mean = np.array([[[0.621247, 0.437153, 0.406354]]])
                # std = np.array([[[0.169446, 0.154287, 0.151743]]])
                # im[:, :, 0] = (im[:, :, 0] - 0.485) / 0.229
                # im[:, :, 1] = (im[:, :, 1] - 0.456) / 0.224
                # im[:, :, 2] = (im[:, :, 2] - 0.406) / 0.225
                # im /= 255.0  # 0 - 255 to 0.0 - 1.0
                cutout = cutout[:, :, ::-1]
                im = get_valid_transforms_cv2(224)(image=cutout)['image']

                ims.append(im)
                nodule_index.append(j)
            pred_cls1_copy = pred_cls1.clone()
            if ims is not None and len(ims):
                ims = torch.stack(ims, dim=0)
                pred_cls2 = model(ims.to(d.device)).argmax(1)
                # pred_cls2 = model(torch.Tensor(ims).to(d.device)).argmax(1)  # classifier prediction
                for n, ni in enumerate(nodule_index):
                    pred_cls1_copy[ni] = pred_cls2[n]
            x[i] = x[i][pred_cls1 == pred_cls1_copy]  # retain matching class detections
    return x
